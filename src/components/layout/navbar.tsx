...
export function Navbar({ user, rates }: NavbarProps) {
  ...
  return (
    <>
      <header className="sticky top-0 z-30 flex flex-col w-full border-b border-amber-200/80 dark:border-slate-800 bg-white/90 dark:bg-slate-950/80 backdrop-blur-xl shadow-2xs transition-colors duration-200">
        {/* Main Navbar Row */}
        <div className="flex h-14 sm:h-16 w-full items-center justify-between px-3 sm:px-4 md:px-8">
          {/* Left: Mobile Menu Toggle & Title */}
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            <button
              onClick={() => setMobileOpen(true)}
              className="rounded-lg p-1.5 sm:p-2 text-slate-500 dark:text-slate-400 hover:bg-amber-50 dark:hover:bg-slate-800 hover:text-amber-800 dark:hover:text-slate-200 md:hidden shrink-0"
            >
            ...
            <div className="flex items-center gap-2 md:hidden min-w-0">
              ...
              />
              <span className="font-serif font-bold text-amber-800 dark:text-amber-400 text-xs sm:text-sm truncate">
                {rates?.shopName || "Zeal Jewellers"}
              </span>
            </div>
          </div>

          {/* Center: Live Rates Banner Ticker (Desktop) */}
          {rates && (
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            <div className="flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded-xl bg-amber-100 dark:bg-amber-500/10 border border-amber-300 dark:border-amber-500/30 text-amber-800 dark:text-amber-400 font-bold text-sm">
              {user.name?.charAt(0).toUpperCase() || "U"}
            </div>
          </div>
        </div>

        {/* Mobile Rates Ticker (shown below navbar on small/medium screens) */}
        {rates && (
          <div className="flex lg:hidden items-center gap-3 overflow-x-auto px-3 sm:px-4 py-1.5 border-t border-amber-100 dark:border-slate-800/60 bg-amber-50/50 dark:bg-slate-900/50 text-[10px] sm:text-[11px] text-slate-700 dark:text-slate-300 scrollbar-none">
            <span className="flex items-center gap-1 font-semibold text-amber-700 dark:text-amber-400 shrink-0">
              <TrendingUp className="h-3 w-3" /> Rates:
            </span>
            <span className="shrink-0">
              22K: <strong className="text-amber-800 dark:text-amber-300">{formatCurrency(rates.goldRate22k)}/g</strong>
            </span>
            <span className="text-amber-300 dark:text-slate-600 shrink-0">•</span>
            <span className="shrink-0">
              18K: <strong className="text-amber-800 dark:text-amber-300">{formatCurrency(rates.goldRate18k)}/g</strong>
            </span>
            <span className="text-amber-300 dark:text-slate-600 shrink-0">•</span>
            <span className="shrink-0">
              Silver: <strong className="text-slate-800 dark:text-slate-200">{formatCurrency(rates.silverRate)}/g</strong>
